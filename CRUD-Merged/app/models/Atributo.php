...
class Atributo{
    ...
    private function inputs(){
        switch(true){
            case $this->tipo=='int':
                return "<input type='number' name='{$this->nome_atributo}' value='<?= \$obj?\$obj['{$this->nome_atributo}']:''?>'><br>\n";
            break;
            case substr($this->tipo,0,7)=='decimal':
                return "<input type='number' step='0.01' name='{$this->nome_atributo}'value='<?= \$obj?\$obj['{$this->nome_atributo}']:''?>'><br>\n";
            break;
            
            case $this->tipo =='text':
            case substr($this->tipo,0,4) =='char':
            case substr($this->tipo,0,7) == 'varchar':
                return "<input type='text' name='{$this->nome_atributo}' value='<?= \$obj?\$obj['{$this->nome_atributo}']:''?>'><br>\n";
            break;
            
            case $this->tipo=='date':
                return "<input type='date' name='{$this->nome_atributo}' value='<?= \$obj?\$obj['{$this->nome_atributo}']:''?>'><br>\n";
            break;

            case $this->tipo=='year':
                return "<input type=\"number\" min=\"0\" max=\"2077\" step=\"1\" value=\"2000\" name='{$this->nome_atributo}' value='<?= \$obj?\$obj['{$this->nome_atributo}']:''?>'>";
            break;
